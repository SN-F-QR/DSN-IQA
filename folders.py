...
class LiveFBFolder(data.Dataset):
    def __init__(self, root, index, transform, patch_num):
        imgname = []
        mos_all = []
        csv_file = os.path.join(root, '640s_labels.csv')
        with open(csv_file) as f:
            reader = csv.DictReader(f)
            for row in reader:
        ...
        self.samples = sample
        self.transform = transform
        self.wPadCrop = PadCenterCrop(size=(640, 640), pad_if_needed=True, fill=255)

    def __getitem__(self, index):
        path, target = self.samples[index]
        sample = pil_loader(path)
        sample = self.wPadCrop(sample)
        sample = self.transform(sample)
        return sample, target
...
# Use to pad the white edges and crop the center
